...
// Défaut métier : (true, true) — exposé par InputOptions.ParDefaut
```
...
public sealed class Channel : AggregateRoot
{
    ...
    public string Nom { get; private set; }              // non vide
    public Platform Type { get; private set; }
    public string BaseUrl { get; private set; }          // invariant : schéma rtmp:// ou rtmps://
    public ProtectedStreamKey Cle { get; private set; }  // JAMAIS le clair

    public static Channel Creer(string nom, Platform type,
        string baseUrl, ProtectedStreamKey cle);
    public void Modifier(string nom, Platform type, string baseUrl);
    public void RemplacerCle(ProtectedStreamKey nouvelleCle);
}
```
...
public sealed record ProtectedStreamKey
{
    ...
    public override string ToString() => Masque;   // pit of success : un log accidentel affiche ****
...
```csharp
public enum SessionStatus { Demarrage, EnCours, Reconnexion, Arrete, Erreur }

public sealed record ReconnectPolicy(
    int MaxTentatives,        // > 0
    TimeSpan DelaiInitial,    // > 0
    double Facteur,           // >= 1.0 (backoff exponentiel)
    TimeSpan DelaiMax);       // >= DelaiInitial
...
public sealed class StreamSession : AggregateRoot
{
    ...
    public SessionStatus Statut { get; private set; }
    public int TentativesReconnexion { get; private set; }
    public ReconnectPolicy PolitiqueReconnexion { get; }
    public string? DerniereErreur { get; private set; }   // toujours passée au scrubbing (§6.3)

    public static StreamSession Lancer(ProfileId profil, ChannelId cible,
        string inputFilePath, ReconnectPolicy politique);       // → Demarrage + SessionLancee

    public void MarquerEnCours();          // Demarrage|Reconnexion → EnCours
    public void EntamerReconnexion(string raison);   // EnCours → Reconnexion (ou → Erreur si tentatives épuisées)
    public void Arreter();                 // Demarrage|EnCours|Reconnexion → Arrete
    public void MarquerEchec(string raison);   // Demarrage|Reconnexion → Erreur
...
|---|---|---|---|
| — (création) | `Demarrage` | `StartStreamCommand` | `SessionLancee` |
| `Demarrage` | `EnCours` | 1ʳᵉ ligne de stats ffmpeg (`frame=`) | `SessionDemarree` |
| `Demarrage` | `Erreur` | échec de lancement (exit précoce, RTMP refusé, GPU indispo) | `SessionEchouee` |
| `Demarrage` | `Arrete` | stop utilisateur pendant le démarrage | `SessionArretee` |
| `EnCours` | `Reconnexion` | chute détectée (exit inattendu du process) | `ReconnexionDemarree` |
| `EnCours` | `Arrete` | stop utilisateur | `SessionArretee` |
| `Reconnexion` | `EnCours` | redémarrage ffmpeg réussi | `SessionRetablie` |
| `Reconnexion` | `Erreur` | `TentativesReconnexion == MaxTentatives` | `SessionEchouee` |
| `Reconnexion` | `Arrete` | stop utilisateur | `SessionArretee` |
| `Arrete` / `Erreur` | — | états terminaux : on relance via une **nouvelle** session | — |

Choix assumé : un échec **initial** (état `Demarrage`) ne déclenche pas le
...
```mermaid
stateDiagram-v2
    [*] --> Demarrage : StartStream
    Demarrage --> EnCours : 1res stats ffmpeg
    Demarrage --> Erreur : échec lancement
    Demarrage --> Arrete : stop utilisateur
    EnCours --> Reconnexion : chute du flux
    EnCours --> Arrete : stop utilisateur
    Reconnexion --> EnCours : redémarrage OK
    Reconnexion --> Erreur : tentatives épuisées
    Reconnexion --> Arrete : stop utilisateur
    Arrete --> [*]
    Erreur --> [*]
```
...
```csharp
public sealed record SessionLancee(SessionId Id, ProfileId Profil, ChannelId Cible, DateTimeOffset OccurredAt) : IDomainEvent;
public sealed record SessionDemarree(SessionId Id, DateTimeOffset OccurredAt) : IDomainEvent;
public sealed record ReconnexionDemarree(SessionId Id, int Tentative, TimeSpan ProchainDelai, string Raison, DateTimeOffset OccurredAt) : IDomainEvent;
public sealed record SessionRetablie(SessionId Id, int ApresTentatives, DateTimeOffset OccurredAt) : IDomainEvent;
public sealed record SessionArretee(SessionId Id, DateTimeOffset OccurredAt) : IDomainEvent;
public sealed record SessionEchouee(SessionId Id, string Raison, DateTimeOffset OccurredAt) : IDomainEvent;
```
...
| `SaveChannelCommand` | `ChannelId?`, nom, type, baseUrl, `string? CleEnClair` | `ChannelId` | clé protégée dans le handler via `IStreamKeyProtector` puis oubliée ; `null` = clé inchangée |
...
| `GetChannelsQuery` | — | `IReadOnlyList<ChannelDto>` — le DTO ne contient **jamais** la clé, seulement `bool CleConfiguree` |
| `GetSessionStatusQuery` | — | `SessionSnapshot?` |
| `GetSessionLogsQuery` | `int MaxLignes` | `IReadOnlyList<string>` (lignes scrubbées) |
...
public enum IndicateurSante { Ok, Alerte }   // Alerte si Speed < 1.0 ou drops croissants

public sealed record SessionSnapshot(SessionId Id, SessionStatus Statut,
    FfmpegStats? Stats, IndicateurSante Sante, int TentativesReconnexion,
...
- S'abonne aux événements du runner :
  - 1ʳᵉ `StatsReceived` → `session.MarquerEnCours()` ;
  - `Exited` inattendu → `session.EntamerReconnexion(raison)` puis relance avec
    backoff exponentiel (`ReconnectPolicy` : délai = min(DelaiInitial × Facteur^n, DelaiMax)) ;
  - succès de relance → `session.MarquerRetablie()` via `MarquerEnCours()`.
...
sequenceDiagram
    ...
    Note over C: StreamSession → Demarrage (SessionLancee)
    R-->>C: StatsReceived (frame=...)
    Note over C: → EnCours (SessionDemarree)
    C-->>UI: ISessionMonitor.Changed(snapshot)
    R-->>C: Exited (inattendu)
    Note over C: → Reconnexion (ReconnexionDemarree)<br/>backoff puis relance
```
...
|---|---|---|---|
| 1 | `Entree.ReadAtNativeRate` | `-re` | si `true` |
| 2 | `Entree.LoopInfinitely` | `-stream_loop -1` | si `true` |
...
`ProtectedStreamKey.Masque` (`****`). `Secrets` = `[ cléEnClair ]`.
...
`OutputLineReceived`, bufferisée, loggée ou insérée dans `DerniereErreur` /
`SessionEchouee.Raison`. Aucun autre composant ne voit de ligne non scrubbée.
Test unitaire dédié.
