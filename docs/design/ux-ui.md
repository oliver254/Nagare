...
3. **Champ « durée » optionnel au lancement** (Lot 1 de la planification, US-0 — ADR-0009) : **livré**. La commande Application attendue existe (`StartStreamCommand.MaxDuration`) ; le tableau de bord porte désormais un `NumberBox` « Durée maximale (heures) » (vide = sans limite, borné par `StreamSession.MaxAllowedDuration`), la légende d'aperçu affiche l'heure de fin **avant** lancement, la carte *Santé* montre l'heure d'arrêt automatique **pendant** la diffusion (depuis `SessionSnapshot.PlannedEndsAt`), et le bilan distingue « arrêtée automatiquement (durée atteinte) » d'un arrêt manuel (`SessionSnapshot.StopReason`). Voir `docs/product/stream-scheduling.md` (US-0) et l'ADR-0009.
